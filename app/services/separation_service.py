...
class ConcurrentSeparationError(SeparationError):
    """Raised when a separation is already in progress on this service."""

    def __init__(self, message: str = "A separation is already in progress") -> None:
        super().__init__(message)
...
class SeparationService:
    ...
    async def initialize(self) -> None:
        ...
        if not self._busy_lock.acquire(blocking=False):
            raise ConcurrentSeparationError()
        try:
    ...
    async def separate(
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Separation complete in {elapsed:.2f}s: {list(result.keys())}"
                )
                return result
